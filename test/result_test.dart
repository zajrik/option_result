...
void main() {
  group('Result:', () {
    ...
    test('Should unwrap values via shorthand getters', () {
      ...
      if (foo case Ok(:int v)) {
        expect(v, equals(1));
      }
      if (foo case Ok(:int val)) {
        expect(val, equals(1));
      }
      if (bar case Err(:String v)) {
        expect(v, equals('bar'));
      }
      if (bar case Err(:String val)) {
        expect(val, equals('bar'));
      }
      if (bar case Err(:String e)) {
        expect(e, equals('bar'));
      }
      if (bar case Err(:String error)) {
        expect(error, equals('bar'));
      }
    });
    ...
    test('Should return expected values from Result#expectErr()', () {
      ...
      expect(
          () => Ok('foo').expectErr('Should be Err()'), throwsA(TypeMatcher<ResultError>()));
    });
    ...
    test('Should return expected values from Result#flatten()', () {
      ...
      expect(
        foo.flatten(),
        equals(Result.from(Result.from(Result.from(1, 'foo'), 'bar'), 'baz')),
      );
